<template>
  <div class="bg-white text-gray-800">
    ...
    <header class="border-b border-gray-200">
      ...
      <div class="bg-gray-100 text-sm py-2">
        <div
          class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center px-4"
        >
          <p v-html="topbarText"></p>
          <p class="mt-1 md:mt-0">
            {{ topbarHelpText }}
            <a
              :href="`tel:${topbarPhoneRaw}`"
      ...
      <!-- Navbar -->
      <div
        class="max-w-7xl mx-auto flex justify-between items-center py-4 px-4"
      >
    ...
    <section class="bg-gray-50 py-12" v-if="offerBanners.length">
      <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-6 px-4">
        <div
          v-for="(banner, i) in offerBanners"
          :key="i"
        ...
        <div class="grid md:grid-cols-2 gap-6">
          ...
          >
            <div class="flex flex-col items-center">
              <img
                :src="deal.image || placeholderImage('product')"
                :alt="deal.name"
            ...
            <div class="text-center md:text-left mb-2">
              ...
              <div class="flex justify-center md:justify-start gap-2 mt-3">
                <div
                  v-for="(unit, i) in deal.timer_units || ['Hours', 'Mins', 'Secs']"
                  :key="i"
    ...
    <section class="py-12 bg-white" v-if="newArrivalsList.length">
      <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          <div
            v-for="(product, index) in newArrivalsList"
            :key="index"
            class="border rounded-xl p-6 hover:shadow-md transition relative flex flex-col items-center text-center"
          >
            <img
    ...
    <section class="py-16 bg-white">
      <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 ">
          ...
          >
            <img :src="featuredCategory.image || placeholderImage('category')" :alt="featuredCategory.name" class="mt-4 w-full rounded-lg object-contain" />
          </div>
    ...
    <section class="bg-[#0D4C3A] py-10 text-white">
      <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <form @submit.prevent="onSubscribe" class="flex items-center w-full md:w-[500px] bg-white rounded-full overflow-hidden border border-white/20 focus-within:ring-2 focus-within:ring-green-500 transition-all">
          <input v-model="newsletterEmail" :placeholder="newsletter.placeholder || 'Your Email Address...'" type="email" class="flex-1 px-5 py-3 text-gray-800 outline-none rounded-l-full" />
          <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-semibold px-8 py-3 rounded-r-full transition-all whitespace-nowrap">
  ...
  <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-10 border-b pb-10">
    ...
    <div>
      ...
      <div class="flex gap-3 mt-4">
        <a
          v-for="(slink, i) in footer?.socials || []"
          :key="i"
          :href="slink.url || '#'"
          :class="slink.class || ''"
          class="text-xl"
  ...
  <div class="max-w-7xl mx-auto px-6 mt-6 text-sm text-gray-500 text-center">
    <div class="flex items-center justify-between flex-wrap gap-4">
      ...
      <img
        :src="footer?.payment_image || placeholderImage('payment')"
        alt="Payment Methods"
...
<style scoped>
/* keep small tweaks if needed; UI matches original Tailwind classes */
</style>
